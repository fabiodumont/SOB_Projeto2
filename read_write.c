...
static void converteParaHexa(unsigned char *in, char *out, int len);
...
static ssize_t inicio_cripto(const char *buffer,size_t len)
{
    ...
    op = buffer[0];
    tamInput = len - 1;
    if(tamInput%32 && op == 'd') return -1;	// Retorna -1 (erro) se a entrada nao for multiplo de 32
    ...
    if(op == 'c') 
	{
        padding(input, tamInput); 						// Faz o padding caso a opcao escolhida seja criptografia
        tamInput += 32 - (tamInput%32); 					// Atualiza a variavel de tamanho apos o padding  
    } 
    ...
    if(op == 'c')
	{
        ...
        crypto=vmalloc(cont*2+1);
        if(!crypto)
		{
            printk(KERN_ERR "kmalloc(crypto) error");
        }

        converteParaHexa(ascii, crypto, cont);					//ascii tem todos os blocos criptografados
        tamSaida = cont*2;
        crypto[cont*2] = '\0';
	...
	else if(op == 'd')
	{
        ...
        decrypto=vmalloc(cont*2);
        if(!decrypto)
		{
            printk(KERN_ERR "kmalloc(crypto) error");
        }

        converteParaHexa(ascii, decrypto, cont);
        tamSaida=cont*2;

        if(unpadding(decrypto, tamSaida) == 0)				//Na descriptografia o unpadding eh feito na saida         
        {
    ...
    printk(KERN_INFO "Recebida mensagem com %ld caracteres!\n", len -1);
    vfree(ascii);
    vfree(input);
...
SYSCALL_DEFINE3(read_crypt, unsigned int, fd, char __user *, buf, size_t, count)
{
	...
	printk("Buffer com o opc: %s \n", bufferOpc);
	
	ret = inicio_cripto(bufferOpc, (count + 1));
	
	memcpy(buf, decrypto, tamSaida);
	printk("Dado decriptado (em Hexa): %s \n", decrypto);
	return 0;
